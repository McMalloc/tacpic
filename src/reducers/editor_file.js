  ...
  switch (action.type) {
    ...
    case OBJECTS_SWAPPED:
      return produce(state, (draftState) => {
        draftState.pages[action.shared_currentPage].objects = move(
    ...
    case "OBJECT_ROTATED":
      ...
      objects = getSelectedObjects(
        oldState.pages[action.shared_currentPage].objects,
        action.uuids
      );
    ...
    case "OBJECT_PROP_CHANGED":
      ...
      return produce(state, (draftState) => {
        filter([].concat.apply([], draftState.pages.map(page => page.objects)), {
          ...
          if (action.prop === "isKey") {
            // && object.keyVal.length === 0) {
    ...
    case BRAILLE_BULK_TRANSLATED:
      return produce(state, (draftState) => {
        draftState.pages.forEach((page, index) => {
          page.objects.forEach((object) => {
            if (object.type === "label") {
              let translated = action.labels.find(
                (label) => label.uuid === object.uuid
              );
              if (translated) {
        ...
        const pages = action.labels.find(
          (label) => label.uuid === "_BRAILLE_PAGES"
        );
        if (pages) {
          draftState.braillePages.braille = pages.braille;
          draftState.braillePages.formatted = wrapAndChunk(
            pages.braille,
            draftState.braillePages.cellsPerRow,
            draftState.braillePages.rowsPerPage
          );
        }
      });
    ...
    case "CHANGE_BRAILLE_PAGE_PROPERTY":
      ...
      return produce(state, (draftState) => {
        ...
        if (width === A4_WIDTH && height === A4_HEIGHT) {
          switch (action.key) {
            case "marginTop":
              if (value + rowsPerPage > A4_MAX_ROWS_PER_PAGE)
                draftState.braillePages.rowsPerPage =
                  A4_MAX_ROWS_PER_PAGE - value;
              break;
            case "rowsPerPage":
              if (marginTop + value > A4_MAX_ROWS_PER_PAGE)
                draftState.braillePages.marginTop =
                  A4_MAX_ROWS_PER_PAGE - value;
              break;
            case "marginLeft":
              if (value + cellsPerRow > A4_MAX_CHARS_PER_ROW)
                draftState.braillePages.cellsPerRow =
                  A4_MAX_CHARS_PER_ROW - value;
              break;
            case "cellsPerRow":
              if (marginLeft + value > A4_MAX_CHARS_PER_ROW)
                draftState.braillePages.marginLeft =
                  A4_MAX_CHARS_PER_ROW - value;
              break;
            case "pageNumbers":
              if (
                value === PAGE_NUMBER_BOTTOM &&
                rowsPerPage + marginTop >= A4_MAX_ROWS_PER_PAGE
              ) {
                // one more row for space when page numbering at the bottom is enabled
                draftState.braillePages.rowsPerPage =
                  draftState.braillePages.rowsPerPage - 1;
              }
              break;
          }
        }
    ...
    case "KEY_TEXTURE_CHANGED":
      return produce(state, (draftState) => {
        const index = draftState.keyedTextures.findIndex(
          (entry) => entry.pattern === action.pattern
        );
        if (index !== -1) {
    ...
    case "OBJECTS_UNGROUPED":
      ...
      objects[groupIndex].objects.forEach((object) => {
        ...
        methods[object.type].translate(
          object,
          objects[groupIndex].x,
          objects[groupIndex].y
        );
      });
